...
# update 7/17/18: this is for predicting Land Cover class in Grand Canyon NP
...
varimportance_cov <- rf.modelSel(rastercolumns, data$Class, imp.scale="se")
...
rastercolumns_cov <- rastercolumns[,as.character(raster_cov_names)[-c(4,7,8,9,10)]]
...
data_cov_model <- cbind(rastercolumns_cov, Class = data$Class)


# Now let's restrict our final predictors to those that are most important (Change manually) 
# and run the continuous model

#rf_model1 = randomForest(Class ~ ., data=data_cov_model, importance = TRUE, ntree = 5000, mtry = 3)
#rf_model1
...
rf_model1<- randomForest(Class ~ Slope + Elevation + NBR + SAVI, importance = TRUE, ntree = 10000, mtry = 2)
rf_model1
